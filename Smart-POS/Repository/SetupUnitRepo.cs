...
{
    internal class SetupUnitRepo : ApiRepository
    {
        public ObservableCollection<SetupUnitListItemModel> GetAll()
        {
            ObservableCollection<SetupUnitListItemModel> list = new ObservableCollection<SetupUnitListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SetupUnitListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupUnitModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupUnitModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupUnitModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/unit", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/unit/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
