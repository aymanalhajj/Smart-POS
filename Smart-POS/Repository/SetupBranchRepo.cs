    ...
    internal class SetupBranchRepo : ApiRepository
    {
        public ObservableCollection<SetupBranchListItemModel> GetAll()
        {
            ...
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SetupBranchListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupBranchModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupBranchModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupBranchModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/branch", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/branch/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
