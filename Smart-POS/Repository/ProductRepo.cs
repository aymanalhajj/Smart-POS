    ...
    internal class ProductRepo : ApiRepository
    {
        public ObservableCollection<ProductListItemModel> GetAll()
        {
            ...
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<ProductListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public ActionStatusModel Post(ProductModel model)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
