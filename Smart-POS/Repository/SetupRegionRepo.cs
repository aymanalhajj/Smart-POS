...
{
    internal class SetupRegionRepo : ApiRepository
    {
        public ObservableCollection<SetupRegionListItemModel> GetAll()
        {
            ObservableCollection<SetupRegionListItemModel> list = new ObservableCollection<SetupRegionListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SetupRegionListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupRegionModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupRegionModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupRegionModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/region", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/region/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
