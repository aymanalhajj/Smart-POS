...
{
    internal class ClientRepo : ApiRepository
    {
        public ObservableCollection<ClientListItemModel> GetAll()
        {
            ObservableCollection<ClientListItemModel> list = new ObservableCollection<ClientListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<ClientListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public ClientModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<ClientModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(ClientModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/client/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
