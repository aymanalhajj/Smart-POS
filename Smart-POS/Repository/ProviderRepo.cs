...
{
    internal class ProviderRepo : ApiRepository
    {
        public ObservableCollection<ProviderListItemModel> GetAll()
        {
            ...
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<ProviderListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/provider/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
