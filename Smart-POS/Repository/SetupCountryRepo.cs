...
{
    internal class SetupCountryRepo : ApiRepository
    {
        public ObservableCollection<SetupCountryListItemModel> GetAll()
        {
            ObservableCollection<SetupCountryListItemModel> list = new ObservableCollection<SetupCountryListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SetupCountryListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupCountryModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupCountryModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupCountryModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/country", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/country/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
