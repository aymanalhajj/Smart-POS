...
{
    internal class SetupProdGroupRepo : ApiRepository
    {
        public ObservableCollection<SetupProdGroupListItemModel> GetAll()
        {
            ObservableCollection<SetupProdGroupListItemModel> list = new ObservableCollection<SetupProdGroupListItemModel>();
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product_groups" +
                ...
                var res = JsonConvert.DeserializeObject<List<SetupProdGroupListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupProdGroupModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupProdGroupModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupProdGroupModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product_group", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product_group/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
