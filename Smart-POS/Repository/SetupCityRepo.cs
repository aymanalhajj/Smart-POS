...
{
    internal class SetupCityRepo : ApiRepository
    {
        public ObservableCollection<SetupCityListItemModel> GetAll()
        {
            ObservableCollection<SetupCityListItemModel> list = new ObservableCollection<SetupCityListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SetupCityListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SetupCityModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SetupCityModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SetupCityModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/city", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/city/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
