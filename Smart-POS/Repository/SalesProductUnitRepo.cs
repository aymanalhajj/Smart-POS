...
namespace Smart_POS.Repository
{
    ...
    {
        public ObservableCollection<SalesProductUnitListItemModel> GetAll()
        {
            ObservableCollection<SalesProductUnitListItemModel> list = new ObservableCollection<SalesProductUnitListItemModel>();
            try
            {
                ...
                var res = JsonConvert.DeserializeObject<List<SalesProductUnitListItemModel>>(response.Content.ReadAsStringAsync().Result);
        ...
        public SalesProductUnitModel Get(string? first, string? last, string? next, string? prev, string? Id)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                ...
                {
                    var res = JsonConvert.DeserializeObject<SalesProductUnitModel>(response.Content.ReadAsStringAsync().Result);
                    return res;
                }
            }
            catch (Exception ex)
        ...
        public ActionStatusModel Post(SalesProductUnitModel model)
        {
            try
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product_unit", UriKind.Absolute);
        ...
        public ActionStatusModel Delete(string Id)
        {
            ...
            {
                var requestUri = new Uri($"{baseUrl}" +
                    $"setup/product_unit/{HttpUtility.UrlEncode(Id)}", UriKind.Absolute);
