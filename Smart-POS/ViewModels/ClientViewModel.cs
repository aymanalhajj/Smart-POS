...
namespace Smart_POS.ViewModels
{
    class ClientViewModel : INotifyPropertyChanged
    {
        ...
        {
            repo = new ClientRepo();
            Client = new ClientItemViewModel();
            InitLists();
        }
        ...
        private ClientRepo repo { get; set; }
        public int CurrentRow { get; set; }
        public int InvoiceToEditIndex { get; set; }
        private ClientItemViewModel client;
        private ObservableCollection<Item> _accountList;
        private ObservableCollection<ClientListItemModel> _ListItems;
        ...
        public void LoadCity()
        {
            ...
            {
                CityList = repo.GetCityList(Client.CountryId.ToString());
            }
        }

        public void LoadRegion()
        {
            if (Client.CountryId != null && Client.CityId != null)
            {
                RegionList = repo.GetRegionList(Client.CountryId.ToString(), Client.CityId.ToString());
            }
        }
        ...
        public ObservableCollection<ClientListItemModel> ListItems
        {
            get
            {
                return _ListItems;
        ...
        private ClientItemViewModel filters;
        ...
        public void ShowData(ClientModel? model)
        {
            if (model != null)
            {
                Client.FromModel(model);
            }
        }
        public void LoadData()
        {
            try
            {
                ...
                {
                    CurrentRow = -1;
                    var res = repo.Get(first: "0", last: "0", next: "0", prev: "0", Id: ListItems[InvoiceToEditIndex].ClientId.ToString());
                    ShowData(res);
                }
            }
        ...
        private void SaveBtnClick()
        {
            ...
            try
            {
                var res = repo.Post(Client.ToModel());
        ...
        private void NextBtnClick()
        {
            try
            {
                if (Client.ClientId != null && !Client.ClientId.Equals("0"))
                {
                    var res = repo.Get(first: "0", last: "0", next: "1", prev: "0", Id: Client.ClientId.ToString());
                    ShowData(res);
                }
            }
            catch (Exception ex)
        ...
        private void PrevBtnClick()
        {
            try
            {
                if (Client.ClientId != null && !Client.ClientId.Equals("0"))
                {
                    var res = repo.Get(first: "0", last: "0", next: "0", prev: "1", Id: Client.ClientId.ToString());
                    ShowData(res);
                }
            }
            catch (Exception ex)
